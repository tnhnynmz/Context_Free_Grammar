...
public class CYK{
    ...
    public static void CYK(String filePath){
        /*if(args.length < 2){ 
        ...
        String strFromCNF = filePath;
        Scanner sc = new Scanner(System.in);
        System.out.println("Please Enter Your String To Test : ");
        String wordString = sc.nextLine();
        sc.close();
        doSteps(strFromCNF,wordString);
    }

    
    public static String[][] callCykTable(){
    ...
    public static void doSteps(String strFromCNF, String wordString){
        parseGrammar(strFromCNF, wordString);
        String[][] cykTable = createCYKTable();
    ...
    public static void parseGrammar(String strFromCNF, String wordString){
        Scanner input = openFile(strFromCNF);
        ...
        while(input.hasNextLine() && line <= 3){
            ...
            tmp.addAll(Arrays.<String>asList(toArray(tempString)));
    ...
    public static void printResult (String[][] cykTable){
        ...
        System.out.println("\nG = (" +  nonTerminals.toString().replace("[", "{").replace("]", "}")
                          + ", " + terminals.toString().replace("[", "{").replace("]", "}")
...
//Jagged Array for the Algorithm
    ...
    public static String[] getAllCombinations2(String[] from, String[] to){
        int length = from.length * to.length;
        int counter = 0;
        String[] combinations = new String[length];
        if(from[0]==""||to[0]==""){ return combinations; };
        for(int i = 0; i < from.length; i++){
            for(int j = 0; j < to.length; j++){
                combinations[counter] = from[i] + to[j];
                counter++;
